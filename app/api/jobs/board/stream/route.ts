...
// whose profile is approved, switched on and not suspended.
export async function GET(req: Request): Promise<Response> {
  ...
  if (!worker || !worker.verified || !worker.active || worker.suspended) {
    return Response.json({ error: "forbidden" }, { status: 403 });
  }
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      const unsubscribe = subscribeJobBoard(send);
